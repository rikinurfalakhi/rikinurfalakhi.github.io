...
function outputSearchResults(results) {
  ...
  // label exact
  if (results.exact.length) label(list, '<p style="background: #ddd;color: #333;padding: 5px;">Defuzzifikasi</p><div class="bg-primary"> </div>');
  // spit out exacts
  ...
  // label fuzzy
  if (results.fuzzy.length) label(list, '<p style="background: #ddd;color: #333;padding: 5px;">Fuzzy Inference Engine</p>');
  // spit out fuzzies
